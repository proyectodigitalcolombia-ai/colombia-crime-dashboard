...
export default function AdminUsuariosPage() {
  ...
  const { data: users = [], isLoading } = useListTransportUsers({ query: { queryKey: ["transcontrol-users"] } });
  const { data: tenants = [] } = useListTenants({ query: { queryKey: ["transcontrol-tenants"], enabled: isSuperadmin } });
  ...
  const handleOpenCreate = () => {
    setEditingUser(null);
    form.reset({ name: "", email: "", password: "", role: "controlador", tenantId: !isSuperadmin ? currentUser.tenantId : null });
    setIsModalOpen(true);
  };
  ...
  const handleOpenEdit = (user: TransportUser) => {
    ...
    form.reset({ name: user.name, email: user.email, password: "", role: safeRole, tenantId: user.tenantId });
    setIsModalOpen(true);
  };
  ...
  const handleToggleActive = (u: TransportUser) => {
    updateMutation.mutate(
      ...
      {
        onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["transcontrol-users"] }); toast({ title: u.isActive ? "Usuario desactivado" : "Usuario activado" }); },
        onError: () => toast({ title: "Error al cambiar estado del usuario", variant: "destructive" }),
      }
    );
  };
  ...
  const onSubmit = (values: UserFormValues) => {
    ...
    if (editingUser) {
      const updateData: Record<string, unknown> = { name: rest.name, email: rest.email, role: rest.role };
      if (isSuperadmin) updateData.tenantId = rest.tenantId;
      if (password) updateData.password = password;
      updateMutation.mutate(
        { userId: editingUser.id, data: updateData },
        {
          onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["transcontrol-users"] }); setIsModalOpen(false); toast({ title: "Usuario actualizado correctamente" }); },
          onError: () => toast({ title: "Error al actualizar el usuario", variant: "destructive" }),
        }
      );
    } else {
      if (!password) { form.setError("password", { message: "La contraseña es requerida para usuarios nuevos" }); return; }
      if (isSuperadmin && !rest.tenantId) { form.setError("tenantId", { message: "Debe seleccionar una empresa" }); return; }
      const createData = {
        name: rest.name, email: rest.email, password,
      ...
      createMutation.mutate(
        { data: createData },
        {
          onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["transcontrol-users"] }); setIsModalOpen(false); toast({ title: "Usuario creado exitosamente" }); },
          onError: () => toast({ title: "Error al crear el usuario", variant: "destructive" }),
        }
      );
    }
  };
  ...
  const tenantMap = useMemo(() => {
    return tenants.reduce<Record<number, string>>((acc, t: Tenant) => { acc[t.id] = t.name; return acc; }, {});
  }, [tenants]);
      ...
      <Card className="shadow-sm border-border/60">
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                <TableRow><TableCell colSpan={isSuperadmin ? 6 : 5} className="text-center py-8 text-muted-foreground">Cargando usuarios...</TableCell></TableRow>
              ) : users.length === 0 ? (
                    ...
                    <TableCell>
                      {u.role !== "superadmin" && (
                        <div className="flex items-center gap-1">
                          <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-primary" onClick={() => handleOpenEdit(u)} title="Editar usuario">
