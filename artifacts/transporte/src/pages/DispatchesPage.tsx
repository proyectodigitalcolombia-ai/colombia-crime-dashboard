...
import { Download, Search, Eye, FilterX, Plus } from "lucide-react";
...
export default function DispatchesPage() {
  ...
  const { data: dispatchesResponse, isLoading } = useListDispatches(queryParams, {
    query: {
      queryKey: ["dispatches", queryParams],
    }
  });
  ...
  const handleExport = () => {
    ...
    XLSX.writeFile(wb, `Despachos_${format(new Date(), "yyyyMMdd")}.xlsx`);
  };
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <Card className="shadow-sm">
          <CardHeader className="p-4 pb-2">
            <CardTitle className="text-xs font-medium text-muted-foreground uppercase">Total Activos</CardTitle>
          </CardHeader>
        ...
        <Card className="shadow-sm border-l-4 border-l-emerald-500">
          <CardHeader className="p-4 pb-2">
            <CardTitle className="text-xs font-medium text-muted-foreground uppercase">A Tiempo</CardTitle>
          </CardHeader>
          <CardContent className="p-4 pt-0">
            <div className="text-2xl font-bold">{summary?.aTime || 0}</div>
          </CardContent>
        </Card>
        <Card className="shadow-sm border-l-4 border-l-amber-500">
          <CardHeader className="p-4 pb-2">
            <CardTitle className="text-xs font-medium text-muted-foreground uppercase">Demorados</CardTitle>
          </CardHeader>
          <CardContent className="p-4 pt-0">
            <div className="text-2xl font-bold">{summary?.demorado || 0}</div>
          </CardContent>
        </Card>
        <Card className="shadow-sm border-l-4 border-l-blue-500">
          <CardHeader className="p-4 pb-2">
            <CardTitle className="text-xs font-medium text-muted-foreground uppercase">Llegados</CardTitle>
          </CardHeader>
          <CardContent className="p-4 pt-0">
            <div className="text-2xl font-bold">{summary?.llegado || 0}</div>
          </CardContent>
        </Card>
        <Card className="shadow-sm border-l-4 border-l-rose-500">
          <CardHeader className="p-4 pb-2">
            <CardTitle className="text-xs font-medium text-muted-foreground uppercase">Salida</CardTitle>
          </CardHeader>
          <CardContent className="p-4 pt-0">
            <div className="text-2xl font-bold">{summary?.salida || 0}</div>
          </CardContent>
        </Card>
      </div>
        ...
        <CardHeader className="p-4 border-b bg-muted/20">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-6 gap-3">
            ...
            <Input 
              ...
              onChange={(e) => setFilters({...filters, generator: e.target.value})}
              className="h-9"
            />
            <Select 
                    ...
                    <TableRow key={dispatch.id} className="hover:bg-muted/30">
                      ...
                      <TableCell>
                        <div className="flex items-center gap-1 text-sm">
                          <span className="truncate max-w-[120px]" title={dispatch.origin}>{dispatch.origin}</span>
                          <span className="text-muted-foreground">→</span>
                          <span className="truncate max-w-[120px]" title={dispatch.destination}>{dispatch.destination}</span>
                        </div>
                      </TableCell>
                      <TableCell className="text-sm truncate max-w-[150px]">{dispatch.generator || "-"}</TableCell>
                      <TableCell>
        ...
        <div className="p-4 border-t text-xs text-muted-foreground flex justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              size="sm"
              className="h-7 px-2 text-xs"
              disabled={currentPage >= Math.ceil((dispatchesResponse?.total ?? 0) / PAGE_SIZE)}
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onCreateDispatch)} className="space-y-4">
              <FormField control={form.control} name="plate" render={({ field }) => (
                <FormItem><FormLabel>Placa *</FormLabel><FormControl><Input {...field} /></FormControl></FormItem>
              )} />
              <div className="grid grid-cols-2 gap-4">
                <FormField control={form.control} name="origin" render={({ field }) => (
                  <FormItem><FormLabel>Origen *</FormLabel><FormControl><Input {...field} /></FormControl></FormItem>
                )} />
              ...
              <FormField control={form.control} name="generator" render={({ field }) => (
                <FormItem><FormLabel>Generador</FormLabel><FormControl><Input {...field} /></FormControl></FormItem>
              )} />
              ...
              <DialogFooter>
                ...
                <Button type="submit" disabled={createDispatchMutation.isPending}>Guardar</Button>
              </DialogFooter>
            </form>
          </Form>
        </DialogContent>
