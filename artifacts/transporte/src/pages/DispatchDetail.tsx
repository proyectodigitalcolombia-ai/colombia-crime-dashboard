...
export default function DispatchDetail() {
  ...
  const { data: dispatch, isLoading: loadingDispatch } = useGetDispatch(dispatchId, {
    query: { queryKey: ["dispatch", dispatchId], enabled: !!dispatchId }
  });

  const { data: checkpoints = [], isLoading: loadingCheckpoints } = useListCheckpoints(dispatchId, {
    query: { queryKey: ["checkpoints", dispatchId], enabled: !!dispatchId }
  });

  const { data: observations = [], isLoading: loadingObservations } = useListObservations(dispatchId, {
    query: { queryKey: ["observations", dispatchId], enabled: !!dispatchId, refetchInterval: 30000 }
  });
  ...
  const handleCreateCheckpoint = () => {
    ...
    createCheckpointMutation.mutate({
      dispatchId,
      data: { location: newCheckpointLocation }
    }, {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: ["checkpoints", dispatchId] });
    ...
    createObservationMutation.mutate(
      {
        ...
        data: {
          ...
          detail: obsDetail.trim()
        }
      },
      {
        onSuccess: () => {
          queryClient.invalidateQueries({ queryKey: ["observations", dispatchId] });
          setObsDetail("");
  ...
  return (
    <div className="space-y-6 max-w-[1400px] mx-auto pb-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Middle & Right: Checkpoints & Observations */}
                        ...
                        <TableRow key={cp.id} className={isCompleted ? "bg-emerald-500/5" : ""}>
                          ...
                          <TableCell className="text-xs text-muted-foreground">
                            <div className="flex flex-col">
                              <span>{cp.plannedDate || "-"}</span>
                              <span>{cp.plannedTime || ""}</span>
                            </div>
                          </TableCell>
                          <TableCell className="text-xs">
                            {cp.adjustedDate ? (
                              <div className="flex flex-col text-blue-600 dark:text-blue-400">
                                <span>{cp.adjustedDate}</span>
                                <span>{cp.adjustedTime || ""}</span>
                              </div>
                            ) : (
                          ...
                          <TableCell className="text-xs">
                            {isCompleted ? (
                              <div className="flex flex-col font-semibold text-emerald-600 dark:text-emerald-400">
                                <span>{cp.realDate}</span>
                          ...
                          <TableCell className="text-xs text-right font-mono">
                            {cp.distanceKm != null ? cp.distanceKm.toFixed(0) : "-"}
                          </TableCell>
                          <TableCell className="text-xs text-right font-mono">
                            {cp.timeHours != null ? cp.timeHours.toFixed(1) : "-"}
                          </TableCell>
                          <TableCell className="text-xs text-right font-mono">
                            {cp.speedKmh != null ? `${cp.speedKmh.toFixed(0)}` : "-"}
                          </TableCell>
                          <TableCell className="text-xs max-w-[120px] truncate" title={cp.novelty || ""}>
                            {cp.novelty || "-"}
                          </TableCell>
                          <TableCell>
                            <Button 
              ...
              <div className="w-full md:w-[320px] shrink-0 p-4 bg-muted/5 flex flex-col">
                ...
                <form onSubmit={handleCreateObservation} className="flex flex-col h-full gap-4">
                  ...
                  <Button 
                    type="submit" 
                    className="w-full" 
                    disabled={createObservationMutation.isPending || !obsDetail.trim()}
                  >
      ...
      <Dialog open={isNewCheckpointModal} onOpenChange={setIsNewCheckpointModal}>
        <DialogContent className="sm:max-w-[425px]">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreateCheckpoint} disabled={createCheckpointMutation.isPending || !newCheckpointLocation}>
              Crear Punto
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={!!activeCheckpoint} onOpenChange={(open) => !open && setActiveCheckpoint(null)}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Registrar Paso por {activeCheckpoint?.location}</DialogTitle>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <Input 
                id="novelty" 
                placeholder="Ej: Retén en la vía, lluvia fuerte..." 
                value={novelty}
