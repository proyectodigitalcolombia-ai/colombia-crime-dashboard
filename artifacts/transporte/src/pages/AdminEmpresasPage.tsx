...
import { Building2, Plus, Edit2, ShieldAlert, ToggleLeft, ToggleRight } from "lucide-react";
  ...
  const handleToggleActive = (t: Tenant) => {
    updateMutation.mutate(
      { tenantId: t.id, data: { isActive: !t.isActive } },
      {
        onSuccess: () => {
          queryClient.invalidateQueries({ queryKey: ["tenants"] });
          toast({ title: t.isActive ? "Empresa desactivada" : "Empresa activada" });
        },
    ...
    <div className="space-y-6">
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
            <DialogTitle>{editingTenant ? "Editar Empresa" : "Crear Nueva Empresa"}</DialogTitle>
          </DialogHeader>
          <Form {...form}>
