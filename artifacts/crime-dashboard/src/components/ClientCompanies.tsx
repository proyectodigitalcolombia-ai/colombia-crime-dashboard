...
/* ── Standalone field — defined OUTSIDE to avoid remount on each render ── */
function FormField({
  label, value, onChange, type = "text", icon, placeholder, multiline,
}: {
  label: string;
  value: string;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      <label style={{
        fontSize: 10, fontWeight: 700, color: E.muted,
...
/* ── Company form — always rendered with its own form instance ── */
function CompanyForm({
  form, onChange, onLogoChange, onLogoRemove, logoRef,
}: {
  form: FormState;
  onChange: (field: keyof FormState, value: string | null) => void;
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
      ...
      <FormField label="NIT" value={form.nit} onChange={v => onChange("nit", v)} />
      <FormField label="Ciudad" value={form.city} onChange={v => onChange("city", v)} />
      <FormField
        label="Dirección" value={form.address}
        onChange={v => onChange("address", v)}
        icon={<MapPin size={11} />}
      />
      <FormField label="Contacto principal" value={form.contact_name} onChange={v => onChange("contact_name", v)} />
      <FormField
        label="Email" value={form.contact_email}
        onChange={v => onChange("contact_email", v)}
        type="email" icon={<Mail size={11} />}
      />
      <FormField
        label="Teléfono" value={form.contact_phone}
        onChange={v => onChange("contact_phone", v)}
        type="tel" icon={<Phone size={11} />}
      />
      <div style={{ gridColumn: "1 / -1" }}>
        <FormField
      ...
      <div style={{ gridColumn: "1 / -1" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          {form.logo ? (
            <>
              <img src={form.logo} alt="Logo" style={{
                height: 48, objectFit: "contain", borderRadius: 6,
                background: "#fff", padding: 4, border: `1px solid ${E.border}`,
              }} />
              <button onClick={onLogoRemove} style={{
                padding: "5px 10px", borderRadius: 6, fontSize: 11,
                background: "transparent", color: E.muted,
                border: `1px solid ${E.border}`, cursor: "pointer",
              }}>
                Quitar logo
              </button>
            </>
...
/* ── Main component ── */
export function ClientCompanies({ dark }: { dark: boolean }) {
  ...
  async function loadCompanies() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(res.status === 401 ? "No autenticado" : body || "Error al cargar empresas");
      }
  ...
  function handleNewChange(field: keyof FormState, value: string | null) {
    setNewForm(f => ({ ...f, [field]: value }));
  }

  function handleEditChange(field: keyof FormState, value: string | null) {
    setEditForm(f => ({ ...f, [field]: value }));
  }

  function handleLogoFile(
    e: React.ChangeEvent<HTMLInputElement>,
    setter: (field: keyof FormState, v: string | null) => void
  ) {
    const file = e.target.files?.[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = ev => setter("logo", ev.target?.result as string);
  ...
  async function handleSave(form: FormState, id: number | "new") {
  ...
  return (
    <div>
      ...
      {editId === "new" && (
        <div style={{
          background: panel, border: `1px solid ${E.cyan}44`,
          borderRadius: 10, padding: 20, marginBottom: 16,
        }}>
          <div style={{ fontSize: 13, fontWeight: 800, color: E.cyan, marginBottom: 16, display: "flex", alignItems: "center", gap: 8 }}>
            <Plus size={14} /> Nueva empresa cliente
          </div>
          <CompanyForm
            form={newForm}
            onChange={handleNewChange}
            onLogoChange={e => handleLogoFile(e, handleNewChange)}
            onLogoRemove={() => handleNewChange("logo", null)}
            logoRef={newLogoRef}
          />
          <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
            <button
              onClick={() => handleSave(newForm, "new")}
              ...
              style={{
                ...
                display: "flex", alignItems: "center", gap: 6,
                opacity: !newForm.name.trim() ? 0.5 : 1,
              }}
            >
      ...
      {loading ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {companies.map(c => (
            <div key={c.id} style={{
              background: panel,
              border: `1px solid ${expanded === c.id ? E.cyan + "44" : border}`,
              ...
              >
                {c.logo ? (
                  <img src={c.logo} alt="Logo" style={{
                    width: 36, height: 36, objectFit: "contain", borderRadius: 6,
                    background: "#fff", padding: 2, flexShrink: 0,
                  }} />
                ) : (
                  <div style={{
                    width: 36, height: 36, borderRadius: 6, background: E.darkBg,
                  ...
                  {editId === c.id ? (
                    <>
                      <CompanyForm
                        ...
                        onLogoChange={e => handleLogoFile(e, handleEditChange)}
                        onLogoRemove={() => handleEditChange("logo", null)}
                        logoRef={editLogoRef}
                      />
                      <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
                        <button
                          onClick={() => handleSave(editForm, c.id)}
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px 24px", fontSize: 12 }}>
                      {c.contact_name && <InfoRow label="Contacto" value={c.contact_name} muted={muted} text={text} />}
                      {c.contact_email && <InfoRow label="Email" value={c.contact_email} muted={muted} text={text} icon={<Mail size={11} />} />}
                      {c.contact_phone && <InfoRow label="Teléfono" value={c.contact_phone} muted={muted} text={text} icon={<Phone size={11} />} />}
                      {c.address && <InfoRow label="Dirección" value={c.address} muted={muted} text={text} icon={<MapPin size={11} />} />}
                      {c.city && <InfoRow label="Ciudad" value={c.city} muted={muted} text={text} />}
                      {c.notes && <InfoRow label="Notas" value={c.notes} muted={muted} text={text} icon={<FileText size={11} />} wide />}
                      <InfoRow label="Registrada" value={new Date(c.created_at).toLocaleDateString("es-CO")} muted={muted} text={text} />
                    </div>
                  )}
                </div>
              )}
...
function InfoRow({ label, value, muted, text, icon, wide }: {
  label: string; value: string; muted: string; text: string;
  icon?: React.ReactNode; wide?: boolean;
}) {
  return (
    <div style={{ gridColumn: wide ? "1 / -1" : undefined }}>
      <div style={{ color: muted, fontSize: 10, textTransform: "uppercase", letterSpacing: "0.06em", display: "flex", alignItems: "center", gap: 4 }}>
        {icon}{label}
      </div>
      <div style={{ color: text, fontWeight: 500, marginTop: 2 }}>{value}</div>
    </div>
  );
}
